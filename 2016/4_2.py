...
def splitString(word):
	s = word.split('-')
	name = ' '.join(s[:-1])
	code = int(s[-1].split('[')[0])
	checksum = s[-1].split('[')[1].replace(']','')
	return (name, code, checksum)
	
def mostCommon(word):
	d = dict()
	for lettre in set(word):
		if lettre == ' ':
			continue
		try:
			d[word.count(lettre)] += lettre
			d[word.count(lettre)] = ''.join(sorted(d[word.count(lettre)]))
		except KeyError:
			d[word.count(lettre)] = lettre
	common = ''
...
def shift(name, code):
	...
	for l in name:
		if l != ' ':
			decrypted += lettres[(lettres.index(l)+code)%26]
		else:
			decrypted += ' '
	return decrypted

for line in data:
	name, code, checksum = splitString(line)
	if isReal(name, checksum):
		if "north" in shift(name, code):
			print(code,shift(name, code))
