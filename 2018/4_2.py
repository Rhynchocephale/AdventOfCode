data = '''[1518-11-07 00:21] falls asleep
...
for line in data:
	if "begins shift" in line:
		currentGuard = int(line.split('#')[1].split()[0])
	if "falls asleep" in line:
		sleepStart = int(line.split(']')[0].split()[1].split(':')[1])
	if "wakes up" in line:
		sleepEnd = int(line.split(']')[0].split()[1].split(':')[1])
		for _ in range(sleepStart, sleepEnd):
			try:
				sleepingLog[currentGuard].append(_)
			except KeyError:
				sleepingLog[currentGuard] = [_]

maxCount = -1
laziestMinute = -1
laziestGuard = -1
for key in sleepingLog:
	maxMinute = most_common(sleepingLog[key])
	if sleepingLog[key].count(maxMinute) > maxCount:
		laziestMinute = maxMinute
		laziestGuard = key
		maxCount = sleepingLog[key].count(maxMinute)

print(laziestGuard)
print(laziestMinute)
print(laziestGuard*laziestMinute)
