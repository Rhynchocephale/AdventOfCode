...
def chemin(W):
	...
	for etape in W:
		direction = etape[0]
		distance = int(etape[1:])
		#right(c2[direction])
		#forward(distance)
		#left(c2[direction])
		for i in range(distance):
			pos.append(vecteurPlus(pos[-1], c[direction]))
	#up()
	#goto(0,0)
	#down()
	del pos[0]
...
def chemin2(W, contacts):
	...
	for etape in W:
		...
		for i in range(distance):
			j+=1
			newPoint = vecteurPlus(pos, c[direction])
			pos = newPoint
			if newPoint in contacts:
				#print(newPoint, j)
				dists.append([newPoint, j])
	#up()
	#goto(0,0)
	#down()
	print('---------------')
...
def addDists(dist1, dist2):
	a = []
	for i in range(len(dist1)):
		a.append([dist1[i][1]+dist2[i][1], dist1[i][0]])
	print(sorted(a))
	

contacts = intersections(chemin(W1),chemin(W2))
print("Step 1: done!")
dist1 = chemin2(W1, contacts)
dist2 = chemin2(W2, contacts)

addDists(dist1, dist2)
#done()
